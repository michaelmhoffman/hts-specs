...
\item \textsf{thickStart}: Start position at which the~\textbf{feature} is visualized with a thicker or accented display.
  ...
  The default value for~\textsf{thickStart} is the value of \textsf{chromStart}.
...
\item \textsf{thickEnd}: End position at which the~\textbf{feature} is visualized with a thicker or accented display.
  ...
  For \textbf{BED7+} \textbf{file}s that are not \textbf{BED8+}, the default value is the value of \textsf{chromEnd} - \textsf{chromStart}.
...
\begin{table}
  \begin{tabularx}{\textwidth}{r L}
    ...
    \bottomrule
  \end{tabularx}
  \caption{table}{\textbf{Custom field data types.}}\label{tab:custom-data-types}
\end{table}
...
This ordering is equivalent to sorting the \textbf{file} using the command \verb|LC\_ALL=C| \verb|sort| \verb|-k 1,1| \verb|-k 2,2n| \verb|-k3,3n|.
...
Tabix is another option for storing larger \ac{BED} \textbf{file}s.\footnote{Li H. (2011). ``Tabix: fast retrieval of sequence features from generic TAB-delimited files.'' \emph{Bioinformatics}, 27(5):718–719. \url{https://doi.org/10.1093/bioinformatics/btq671}}
