...
The scope of this specification is to formalize reasonable interpretations of the \ac{BED} format.
This specification also aims to make clear interoperability issues that currently cause confusion and can be better specified in a future specification.
...
\begin{tabularx}{\textwidth}{r L L}
  ...
  \bottomrule
\end{tabularx}
\captionof{table}{\textbf{Typographic conventions}}
...
\item[field separator:]
  ...
  However, some capabilities of the \ac{BED} format are only available if a single tab is used as the \textbf{field} separator throughout the file.
...
\item[line separator:]
  Either carriage return, newline, or carriage return followed by newline.
  A carriage return is defined as~`\texttt{{\textbackslash}r}', equivalent to~`\texttt{{\textbackslash}x0d}'.
  A newline is defined as~`\texttt{{\textbackslash}n}', equivalent to~`\texttt{{\textbackslash}x0a}'.
  A carriage return followed by a newline is defined as~`\texttt{{\textbackslash}r{\textbackslash}n}', equivalent to~`\texttt{{\textbackslash}x0d{\textbackslash}x0a}'
...
\end{adjustwidth}
\captionof{table}{\textbf{\ac{BED} fields}}
...
\item \textsf{name}: String that describes the~\textbf{feature}.
  ...
  \textsf{name} does not need to be unique.
...
\item \textsf{thickStart}: Start position at which the~\textbf{feature} is visualized with a thicker or accented display.
  ...
  The default value for~\textsf{thickStart} is \texttt{0}.
...
\item \textsf{thickEnd}: End position at which the~\textbf{feature} is visualized with a thicker or accented display.
  ...
  For \textbf{BED7+} \textbf{file}s that are not \textbf{BED8+}, the default value is \textsf{chromEnd} - \textsf{chromStart}.
...
Each custom \textbf{field} should contain either one of the following data types or a comma-separated list of values of the same type, except for strings:
...
\begin{tabularx}{\textwidth}{r L}
  ...
  Type & Definition \\
  \midrule
  Integer & Textual representation of 64-bit signed integer\\
  Unsigned & Textual representation of 64-bit unsigned integer \\
  Float & Textual representation of 64-bit floating point number\footnote{\emph{IEEE 754--1985 IEEE Standard for Binary Floating-Point Arithmetic.} IEEE 754--1985, 1985} \\
  Character & One character, other than tab \\
  String & One or more characters, other than tab \\
  \bottomrule
\end{tabularx}
\captionof{table}{\textbf{Custom field data types}}
